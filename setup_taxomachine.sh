...
#	echo "$TAXOMACHINE_START_SCRIPT makegenusindexes $TAXOMACHINE_DB"
	...
	$TAXOMACHINE_START_SCRIPT loadtaxsyn $OTT_SOURCENAME $OTT_TAXONOMY $OTT_SYNONYMS $TAXOMACHINE_DB || exit	
	$TAXOMACHINE_START_SCRIPT makecontexts $TAXOMACHINE_DB || exit
	$TAXOMACHINE_START_SCRIPT makegenusindexes $TAXOMACHINE_DB || exit
fi
